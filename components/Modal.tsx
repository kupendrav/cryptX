...
export function Modal({ title, body, onClose }: ModalProps) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center" onKeyDown={(e) => e.key === "Escape" && onClose()} tabIndex={-1}>
      ...
      <div className="relative z-10 w-[92vw] max-w-2xl rounded-2xl border border-white/10 bg-black/60 backdrop-blur-2xl shadow-2xl p-8 opacity-100 scale-100 transition">
        <h3 className="m-0 text-3xl md:text-4xl font-bold">{title}</h3>
        <pre className="whitespace-pre-wrap mt-4 text-slate-100/90 text-xl">{body}</pre>
